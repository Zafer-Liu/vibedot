...
状态特征 `...0003`（read/notify，4 B）：`[state, rx_hi, rx_lo, batt]`，
state 1=已刷新 / 0=帧不完整（rx_hi/rx_lo 为已收字节数，PC 据此整帧重传 ≤3 轮）；
batt = 设备电量 %（v9，0-100；未识别电池通道时固定 100）。
...
| v9 | **电量检测**：原厂固件有电池检测硬件（逆向见 `[BATTERY]`/`VBUS`/`adc_oneshot`/`ESP_SLEEP_VBAT_POWER_DEEPSLEEP_MODE` 字符串，原厂低电量会自动深睡）但采样代码不在此固件变体、电池 ADC 引脚未知。方案：空闲 ADC1 CH0-2（GPIO0/1/2）三路全采，取落在合理锂电范围（直连 2.8-4.3V 或二分频 1.4-2.3V）的通道，16 次均值，30s 周期刷新；电量经状态特征第 4 字节上报 + 串口 `[BATT]` 打印（USB 直连时服务器也解析）。**广播自愈**：断连即重播 + 未连接时每 10s 看门狗式重启广播（兜底协议栈异常/默认广播超时等"蓝牙莫名关闭"）。**复位原因打印**：启动横幅带 `esp_reset_reason()`（brownout=欠压掉电是"蓝牙消失"的高嫌疑原因） |
